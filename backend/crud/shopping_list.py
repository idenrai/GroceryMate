...
def create_shopping_list(db, shopping_list: ShoppingListCreate, user_id: int):
    shopping_list_id = uuid4()
    db.execute(
        "INSERT INTO ShoppingLists (list_id, user_id, list_name, total_estimated_price) VALUES (?, ?, ?, ?)",
        (shopping_list_id, user_id, shopping_list.name, 0.0),
    )
    for item in shopping_list.items:
        item_id = uuid4()
        db.execute(
            "INSERT INTO ShoppingListItems (item_id, list_id, product_id, quantity, estimated_price) VALUES (?, ?, ?, ?, ?)",
            (item_id, shopping_list_id, item.product_name, item.quantity, 0.0),
        )
    return ShoppingList(
...
def update_shopping_list(db, shopping_list_id: str, shopping_list: ShoppingListCreate):
    db.execute(
        "UPDATE ShoppingLists SET list_name = ? WHERE list_id = ?",
        (shopping_list.name, shopping_list_id),
    )
    db.execute(
